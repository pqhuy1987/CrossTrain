...
struct Bank {
    ...
    static func storeworld(_ w:Int) {
        self.whichworld = w
    }
    
    //------------Scenes------------
    static func getScene(_ x:Int) -> SKScene {
        //Get the scene
    ...
    static func storeScene(_ aScene:SKScene, x:Int) {
        //Push the scene
        if(x == 1){
    ...
    static func storepreviousscene(_ x:Int) {
        self.previous_scene = x
    }
    
    //-----------------screen shot------------------
    //store number of level from 1 to
    static func getshot(_ num:Int) -> UIImage {
        //num is level of 关
        return sc_shot[num - 1]
    }
    
    static func storeshot(_ im:UIImage, num:Int) {
        sc_shot[num - 1] = im
    }
}
