...
class Mainplay: SKScene {
    ...
    var timer_L:Timer!    //Timer for the cars
    var timer_R:Timer!
    var timer_train:Timer!     //Timer for the next train
    var timer_remain:Timer!   //Add cars according to level
    var timer_sec:Timer!      //Count every second
    ...
    override func didMove(to view: SKView) {
        ...
        if(world != ww){
            ...
            if(did_ornot == true){
                ...
                //back_components3.runAction(SKAction.rotateByAngle(1.57, duration: 0.0)); //让他竖起来
                back_components3.position = CGPoint(x:self.frame.midX - 245, y:self.frame.midY - 10)
                ...
                var swipe_up = UISwipeGestureRecognizer(target: self, action: #selector(Mainplay.ges_up))
                swipe_up.direction = UISwipeGestureRecognizerDirection.up
                self.view?.addGestureRecognizer(swipe_up)
                var swipe_down = UISwipeGestureRecognizer(target: self, action: #selector(Mainplay.ges_down))
                swipe_down.direction = UISwipeGestureRecognizerDirection.down
                self.view?.addGestureRecognizer(swipe_down)
                
                self.isPaused = false
                
                print(time_left_train)
                timer_train = Timer.scheduledTimer(
                    timeInterval: TimeInterval(time_left_train),
                    target: self,
                    selector: #selector(Mainplay.tcom),
                    userInfo: nil,
                    repeats: false
                )
                
                //This timer will count for when next car arrives
                timer_L = Timer.scheduledTimer(
                    timeInterval: 3,
                    target: self,
                    selector: #selector(Mainplay.result_L),
                    userInfo: nil,
                    repeats: true
                )
                
                timer_R = Timer.scheduledTimer(
                    timeInterval: 3,
                    target: self,
                    selector: #selector(Mainplay.result_R),
                    userInfo: nil,
                    repeats: true
                )
                
                timer_remain = Timer.scheduledTimer(
                    timeInterval: 4,
                    target: self,
                    selector: #selector(Mainplay.add_car),
                    userInfo: nil,
                    repeats: true
                )
                
                timer_sec = Timer.scheduledTimer(
                    timeInterval: 1,  //count for every seconds
                    target: self,
                    selector: #selector(Mainplay.count_sec),
                    userInfo: nil,
                    repeats: true
                )
                ...
                backgroundS.position = CGPoint(x:self.frame.midX, y:self.frame.midY)
                ...
                truck_L.position = CGPoint(x:self.frame.midX - 98, y:self.frame.midY)
                truck_L.zPosition = 0
                ...
                truck_R.position = CGPoint(x:self.frame.midX + 20, y:self.frame.midY)
                truck_R.zPosition = 0
                ...
                back_components1.position = CGPoint(x:self.frame.midX, y:self.frame.midY + 120)
                ...
                back_components2.position = CGPoint(x:self.frame.midX - 80, y:self.frame.midY - 30)
                ...
                back_components4.position = CGPoint(x:self.frame.midX - 385, y:self.frame.midY - 120)
                ...
                back_icon.position = CGPoint(x:self.frame.midX - 460, y:self.frame.midY + 235)
                ...
                timer_train = Timer.scheduledTimer(
                    timeInterval: TimeInterval(time_rand),
                    target: self,
                    selector: #selector(Mainplay.tcom),
                    userInfo: nil,
                    repeats: false
                ...
                timer_remain = Timer.scheduledTimer(
                    timeInterval: 4,  //Uset this to control traffic flow
                    target: self,
                    selector: #selector(Mainplay.add_car),
                    userInfo: nil,
                    repeats: true
     ...
     func touchesEnded(_ touches: NSSet, with event: UIEvent) {
        ...
        if(tnode.isEqual(back_icon)){
            ...
            UIGraphicsBeginImageContext(CGSize(width: 1136, height: 640)) //Create a image in the size of the view
            self.view?.drawHierarchy(in: CGRect(x: 0.0, y: 0.0, width: 1136, height: 640), afterScreenUpdates: true)
    ...
    func ges_up() {
        ...
        if(GanZi_Up == -1){
            ...
            GanZi_Up = 5
            back_components3.run(SKAction.rotate(byAngle: 1.57, duration: 2), completion: {()
                self.GanZi_Up = 1
    ...
    func MovementSet(_ lorr:Int) {
        ...
        if(lorr == 0){
            ...
            truck1.run(SKAction.move(to: up, duration: traffic_time), completion: {()
            ...
            truck2.run(SKAction.move(to: down, duration: traffic_time), completion: {()
    ...
    func tcom() {
        ...
        if(randnum_way == 0){
            //Up way
            track2.position = CGPoint(x:self.frame.midX - 1200, y:self.frame.midY + track_position_up)
            track_ins.LetTrainGo(self.world, train: randnum_kind, trainway: track2, zhengfan: 0)
        }else if(randnum_way == 1){
            //Down way
            track1.position = CGPoint(x:self.frame.midX + 1200, y:self.frame.midY + track_position_down)
    ...
    func result_L() {
        if(GanZi_Up != 1){
            let numchild = truck_L.children.count
            if(numchild < 3){
                self.MovementSet(0)
            }
        }else{
    ...
    func result_R() {
        if(GanZi_Up != 1){
            let numchild = truck_R.children.count
            if(numchild < 2){
                self.MovementSet(1)
            }
        }else{
